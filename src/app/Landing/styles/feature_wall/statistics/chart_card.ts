...
export default () => {
  ...
  return {
    wrapper: cn(
      'absolute w-48 h-36 rounded-xl top-6 right-6 border border-dotted',
      'bg-cover bg-center',
      bg('htmlBg'),
    ...
    trendText: cn('absolute top-2 left-3 text-xs scale-90', fg('text.hint')),
    trendNum: cn('absolute top-7 left-3.5 text-base', fg('text.title')),
    //
    bottomGradient: 'absolute w-full h-14 bottom-0 right-0 rounded-xl z-30',
    bottomGradientBg: 'linear-gradient(360deg, rgb(255 255 255) 5%, rgb(0 0 0 / 0%) 80%)',

    topGradient: 'absolute w-2/3 h-10 top-0 right-0 rounded-xl z-30',
    topGradientBg: 'linear-gradient(rgb(255 255 255) 25%, rgb(0 0 0 / 0%) 90%)',
    //
    highlightColumn: cn(
      'absolute right-24 mr-1 top-10 w-0.5 h-32 z-20 opacity-40 trans-all-200',
      rainbow(COLOR_NAME.ORANGE, 'bg'),
    ),
    highlightColumnHover: 'opacity-100 right-16 mr-3',

    highlightDot: cn(
      'absolute size-3 circle top-24 mt-2 right-24 trans-all-200 z-40 opacity-65',
      'border-2',
      rainbow(COLOR_NAME.ORANGE, 'bg'),
    ),
    ...
    //
    column: cn('absolute top-12 w-px h-28 opacity-20 z-10 trans-all-100', bg('text.digest')),
  }
}
