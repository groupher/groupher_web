  ...
  return (
    <Wrapper>
      {!includes(article.id, demoList) ? (
        <ArticleCatState cat={ARTICLE_CAT.QUESTION} />
      ) : (
        <Fragment>
          {article.id === '239' && (
            <ArticleCatState cat={ARTICLE_CAT.FEATURE} />
          )}
          {article.id === '231' && <ArticleCatState cat={ARTICLE_CAT.BUG} />}
          {article.id === '227' && (
            <ArticleCatState cat={ARTICLE_CAT.BUG} state="TODO" />
          )}
          {article.id === '228' && (
            <ArticleCatState cat={ARTICLE_CAT.FEATURE} state="WIP" />
          )}
          {article.id === '226' && (
            <ArticleCatState cat={ARTICLE_CAT.QUESTION} state="RESOLVE" />
          )}
          {article.id === '225' && (
            <ArticleCatState
